...
function init() {
    ...
    document.getElementById("signup-btn").onclick = async function () {
        ...
        await fetch("http://72.167.133.198:8000/signup/", {
            method: 'POST',
            headers: {
...
function rearrangeTable() {
    ...
    if (width >= 630) {
        ...
        document.getElementById("list-title").innerHTML = "Signed up:";
        document.getElementById("signedup").innerHTML = "<tr><th><b>#</b></th><th><b>Name<br>(Role)</b></th></tr>";
        for (let i = 0; i < persons.length - 1; i++) {
            if (signedup[i][1].length > 0) {
                document.getElementById("signedup").innerHTML += "<tr><th>" + (i + 1) + "</th><th>" + signedup[i][0] + "<br>(" + signedup[i][1] + ")</th></tr>";
            } else {
                document.getElementById("signedup").innerHTML += "<tr><th>" + (i + 1) + '</th><th style="height: 54px;">' + signedup[i][0] + "</th></tr>";
            }
        }
    }
}